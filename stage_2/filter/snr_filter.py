...
def calculate_wada_snr(audio):
    "Функция Вычисляет SNR компоненты для аудио"
    # Init constants
    eps = 1e-10
    db_vals = np.arange(-20, 101)
    g_vals = np.array([0.40974774, 0.40986926, 0.40998566, 0.40969089, 0.40986186, 0.40999006, 
                       ...
                       1.62632463, 1.6274027, 1.62842767, 1.62945532, 1.6303307, 1.63128026, 1.63204102])

    # Усредняем стерео каналы в моно
    if audio.ndim == 2:
        audio = np.mean(audio, axis=1)

    # Peak normalize, get magnitude, clip lower bound
    audio = audio.astype(float) / abs(audio).max()  # Предотвращение ошибок с int16
    abs_audio = np.abs(audio)
    abs_audio[abs_audio < eps] = eps

    # Calculate statistics
    # E[|z|]
    v1 = max(eps, abs_audio.mean())
    # E[log|z|]
    v2 = np.log(abs_audio).mean()
    # log(E[|z|]) - E[log(|z|)]
    v3 = np.log(v1) - v2
    ...
    snr_idx = None
    if any(g_vals < v3):
        snr_idx = np.where(g_vals < v3)[0].max()

    # Handle edge cases or interpolate
    if snr_idx is None:
        wav_snr = db_vals[0]
    elif snr_idx == len(db_vals) - 1:
        wav_snr = db_vals[-1]
    else:
        wav_snr = db_vals[snr_idx] + (v3 - g_vals[snr_idx]) / (g_vals[snr_idx + 1] - g_vals[snr_idx]) * (db_vals[snr_idx + 1] - db_vals[snr_idx])
